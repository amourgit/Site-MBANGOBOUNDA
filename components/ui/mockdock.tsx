...
function DockItem({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Vérifier si la souris est réellement sur l'élément
      const rect = ref.current.getBoundingClientRect();
      const isInside = orientation === "vertical"
      ...
      if (!isInside) {
        mouseXMotion.set(1000); // Valeur éloignée pour désactiver l'effet
        return;
      }
      
      // Utiliser clientY pour vertical, clientX pour horizontal
      const mousePos = orientation === "vertical" ? e.clientY : e.clientX;
      const rectPos = orientation === "vertical" 
        ? rect.y + rect.height / 2 
        : rect.x + rect.width / 2;
      
      const distance = mousePos - rectPos;
  ...
  }, [mouseXMotion, isTouchDevice, orientation]);
  ...
  return (
    <motion.div
      style={{ 
        ...
        width: orientation === "vertical" ? "auto" : undefined,
        scrollbarWidth: "none" 
      }}
      className={cn(
        "flex items-center",
        orientation === "vertical" ? "flex-col" : "mx-2 max-w-full"
      )}
    >
      <motion.div
        ...
        className={cn(
          `flex items-end w-fit rounded-full border-white/20 border-2 ${getBgStyles()}`,
          orientation === "vertical" 
            ? "flex-col gap-2 py-2 px-1" 
            : "gap-4 pb-2 px-4 absolute bottom-2 left-1/2 transform -translate-x-1/2",
          className
        )}
        style={{ height: orientation === "vertical" ? "auto" : panelHeight }}
        role="toolbar"
