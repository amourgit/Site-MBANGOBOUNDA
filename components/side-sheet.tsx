...
import FacenoteGlassSidebar from "./widgets/side-items";
...
const SideSheetContent = ({
  ...
  const calculateWidth = useCallback(() => {
    if (typeof window !== "undefined") {
      ...
      // Sur mobile (≤ 640px), prendre 85% de la largeur au lieu de 90%
      if (vw <= 640) {
        calculatedWidth = vw * 0.85;
      }
      // Sur tablette (≤ 1024px), prendre 60% au lieu de 70%
      else if (vw <= 1024) {
        calculatedWidth = vw * 0.6;
      }
      // Sur desktop, garder 320px fixe
      else {
        calculatedWidth = 320;
      }

      // Largeur minimale pour éviter les très petits écrans
  ...
  const getInitialX = useCallback(() => {
    return side === "left" ? -sheetWidth : sheetWidth;
  }, [side, sheetWidth]);
  ...
  const handleDragEnd = useCallback(
    (_event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
      const threshold = 320 * closeThreshold;
    ...
    [controls, onClose, closeThreshold, side]
  );
    ...
    <SideSheetPortal>
      ...
      >
        ...
        <motion.div
          ...
          className={cn(
            "absolute bg-transparent backdrop-blur-xl dark:bg-[#0A0A0A] shadow-2xl border border-gray-800 pr-0",
        ...
        >
          <div className="h-full overflow-hidden">
            <div
              className="h-full overflow-y-hidden px-6 scrollbar-hide"
              style={{
                scrollbarWidth: "none",
                msOverflowStyle: "none",
              }}
            >
              <FacenoteGlassSidebar 
                onMenuItemClick={(item) => {
                  console.log('Menu item clicked:', item.title, item.link);
                }}
              />
            </div>
          </div>
