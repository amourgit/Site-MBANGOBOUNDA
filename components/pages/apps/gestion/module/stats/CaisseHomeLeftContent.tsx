...
const SaleItem = ({ sale, isActive, onClick, onDelete, onEdit, onStatusChange, dragHandleProps }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`group relative bg-white rounded-lg border-2 transition-all duration-300 cursor-pointer mb-2 ${
        isActive 
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between mb-2">
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-600 line-clamp-2">{sale.description}</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 mb-2">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-gray-600">{sale.client}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Calendar className="w-3 h-3 text-gray-400" />
              <span className="text-[10px] text-gray-600">{sale.dateCreation}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Package className="w-3 h-3 text-gray-400" />
              <span className="text-[10px] text-gray-600">{totalItems} article{totalItems > 1 ? 's' : ''}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <DollarSign className="w-3 h-3 text-gray-400" />
              <span className="text-[10px] font-semibold text-blue-600">{totalAmount.toFixed(2)} €</span>
            </div>
          </div>

          <div className="flex items-center gap-1 mb-2">
            <span className="text-[9px] text-gray-500">Paiement:</span>
    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          />
          <div className="absolute top-full left-0 mt-1 w-44 bg-white rounded-lg shadow-xl border border-gray-200 z-20 max-h-56 overflow-y-auto">
            <div className="p-1">
              {options.map((option) => (
                <button
                  key={option.value}
...
export default function CaisseLeftSidebar({ 
  ...
  const filteredSales = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(sale => 
        sale.numeroVente.toLowerCase().includes(query) ||
  ...
  const handleCreateSale = () => {
    const newSale = {
      id: Date.now(),
      numeroVente: `2024-${String(sales.length + 1).padStart(3, '0')}`,
    ...
    <div className="h-full bg-white/95 backdrop-blur-sm flex flex-col">
      <div className="p-3 border-b border-gray-200">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-sm font-bold text-gray-900">Ventes</h2>
          <button
        ...
        <div className="relative mb-2">
          ...
          <input
            ...
            className="w-full pl-7 pr-2 py-1 border border-gray-300 rounded-md text-[11px] focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-1">
            ...
            <span className="text-[10px] font-medium text-gray-700">Filtres</span>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-2.5">
        {filteredSales.length === 0 ? (
          <div className="text-center py-8 animate-fadeIn">
            ...
            <p className="text-xs text-gray-500">Aucune vente trouvée</p>
            {hasActiveFilters && (
              <button
                onClick={() => {
                  setSearchQuery("");
